...
export class AdminController {
    ...
    @ApiOperation({
        summary: 'Ban a user',
        description: 'Bans a user account with optional reason and expiration. Admin only.',
    })
    ...
    @ApiOperation({
        ...
        description: 'Removes ban from a user account. Admin only.',
    })
    @ApiParam({ name: 'id', description: 'User ID', type: Number })
    @ApiResponse({
        status: HttpStatus.OK,
    ...
    @ApiOperation({
        ...
        description: 'Revokes a specific session by ID. Admin only.',
    })
    ...
    @ApiOperation({
        ...
        description: 'Revokes all active sessions for a specific user. Admin only.',
    })
    @ApiParam({ name: 'id', description: 'User ID', type: Number })
    @ApiResponse({
        status: HttpStatus.OK,
    ...
    @ApiOperation({
        ...
        description: 'Retrieves comprehensive dashboard statistics including user metrics, session analytics, gate usage, and role distribution. Admin only.',
    })
    @ApiResponse({
        status: HttpStatus.OK,
            ...
            properties: {
                users: {
                    ...
                    properties: {
                        ...
                        banned: { type: 'number' },
                        active: { type: 'number' },
                        recent: { type: 'number', description: 'Users created in last 7 days' },
                    },
                },
                sessions: {
                    type: 'object',
                    properties: {
                        ...
                        revoked: { type: 'number' },
                        expired: { type: 'number' },
                        recent: { type: 'number', description: 'Sessions created in last 24 hours' },
                    },
                },
                gates: {
                    type: 'object',
    ...
    async getDashboard() {
        ...
        const [
            ...
            this.prisma.session.count({ where: { revoked: false, expiresAt: { gt: now } } }),
            this.prisma.session.count({ where: { revoked: true } }),
            this.prisma.session.count({ where: { revoked: false, expiresAt: { lte: now } } }),
