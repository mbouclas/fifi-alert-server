...
export class DeviceService {
        ...
        if (existing) {
            ...
            await this.prisma.device.update({
                ...
                data: {
                    ...
                    push_token_updated_at: dto.push_token ? now : existing.push_token_updated_at,
                    last_app_open: now,
    ...
    getLocationStatus(device: Device, savedZoneCount: number): LocationStatusDto {
        const hasGps = device.gps_latitude !== null && device.gps_longitude !== null;
        const hasIpLocation = device.ip_latitude !== null && device.ip_longitude !== null;
        ...
        if (hasGps && device.gps_updated_at) {
            gpsAgeHours = (Date.now() - device.gps_updated_at.getTime()) / (1000 * 60 * 60);
     ...
     */
    private mapToResponseDto(device: Device & { saved_zones?: any[] }): DeviceResponseDto {
