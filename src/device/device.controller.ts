...
export class DeviceController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieve all devices registered to the current user, ordered by last app open.',
  })
  @ApiResponse({
    status: 200,
  ...
  async updatePushToken(
    ...
    return this.deviceService.updatePushToken(
      id,
      body.push_token,
      session.userId,
    );
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid input or max zones exceeded',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 404, description: 'Device not found' })
  ...
  @ApiOperation({
    ...
    description:
      'Update name, radius, priority, or active status of a saved zone.',
  })
  @ApiParam({
    name: 'zoneId',
