...
export class RateLimitService {
  ...
  async checkAlertCreationLimit(userId: number): Promise<void> {
    ...
    try {
      ...
      const hourlyCount = this.countInWindow(
        timestampNumbers,
        now,
        this.limits.hourly.windowMs,
      );
      const dailyCount = this.countInWindow(
        timestampNumbers,
        now,
        this.limits.daily.windowMs,
      );
      const weeklyCount = this.countInWindow(
        timestampNumbers,
        now,
        this.limits.weekly.windowMs,
      );
   ...
   */
  private async recordAlertCreation(
    userId: number,
    timestamp: number,
  ): Promise<void> {
    const key = `rate_limit:alert:user:${userId}`;
    ...
    await this.redis.expire(
      key,
      Math.ceil(this.limits.weekly.windowMs / 1000) + 86400,
    );

    this.logger.debug(
      `Recorded alert creation for user ${userId} at ${timestamp}`,
    );
  }
  ...
  private getRetryAfter(
    ...
    const cutoff = now - windowMs;
    const relevantTimestamps = timestamps
      .filter((t) => t >= cutoff)
      .sort((a, b) => a - b);
  ...
  async getUserStats(userId: number): Promise<{
    ...
    return {
      hourly: this.countInWindow(
        timestampNumbers,
        now,
        this.limits.hourly.windowMs,
      ),
      daily: this.countInWindow(
        timestampNumbers,
        now,
        this.limits.daily.windowMs,
      ),
      weekly: this.countInWindow(
        timestampNumbers,
        now,
        this.limits.weekly.windowMs,
      ),
