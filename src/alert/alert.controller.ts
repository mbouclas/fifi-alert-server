import { Controller, Get, Post, Patch, Body, Param, Query, UseGuards, HttpCode, HttpStatus, NotFoundException, ParseIntPipe, UseInterceptors, UploadedFiles } from '@nestjs/common';
...
export class AlertController {
    ...
    async findOne(
        ...
        const alert = await this.alertService.findById(id, requesterId);
    ...
    @ApiResponse({ status: 200, description: 'Alerts found', type: [AlertResponseDto] })
    async findAll(
    ...
    @ApiResponse({ status: 200, description: 'Alert updated successfully', type: AlertResponseDto })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiResponse({ status: 403, description: 'Forbidden - not the alert creator' })
    @ApiResponse({ status: 404, description: 'Alert not found' })
    async update(
        @Param('id', ParseIntPipe) id: number,
        @Session() session: any,
    ...
    @ApiResponse({ status: 200, description: 'Alert renewed successfully', type: AlertResponseDto })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    @ApiResponse({ status: 403, description: 'Forbidden - not the alert creator' })
    @ApiResponse({ status: 404, description: 'Alert not found' })
    ...
    @ApiParam({ name: 'id', description: 'Alert ID' })
    @ApiOperation({ summary: 'Upload photos for an alert (max 5 files, 10MB each)' })
    @ApiResponse({ status: 200, description: 'Photos uploaded successfully', schema: { example: { photoUrls: ['http://localhost:3000/uploads/alerts/1234567890-photo1.jpg'] } } })
