...
describe('Geospatial Queries (e2e)', () => {
  ...
  const NYC_COORDS = { lat: 40.7128, lon: -74.006 }; // Times Square
  ...
  beforeAll(async () => {
    ...
    await prisma.user.deleteMany({
      where: { email: { contains: 'geospatial-test' } },
    });
  ...
  describe('Alert Search API with Geospatial Filtering', () => {
    ...
    it('should find only NYC alert within 5km of NYC', async () => {
      ...
      const nycAlerts = res.body.alerts.filter(
        (a: any) => a.petName === 'NYC Dog',
      );
      expect(nycAlerts.length).toBeGreaterThanOrEqual(1);

      const brooklynAlerts = res.body.alerts.filter(
        (a: any) => a.petName === 'Brooklyn Cat',
      );
