...
describe('Location & Geospatial Integration Tests (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    geospatialService = moduleFixture.get<GeospatialService>(GeospatialService);
  ...
  async function cleanupTestData() {
    await prisma.savedZone.deleteMany({
      where: {
        device_id: { in: [testDevice1Id, testDevice2Id, testDevice3Id] },
      },
    });
  ...
  describe('LocationService - Device Matching Integration', () => {
    it('should find device with fresh GPS near alert location', async () => {
      ...
      const device1Match = matches.find(
        (m) => m.deviceId === testDevice1Id.toString(),
      );
    ...
    it('should find device with postal code match', async () => {
      ...
      const device2Match = matches.find(
        (m) => m.deviceId === testDevice2Id.toString(),
      );
    ...
    it('should NOT match distant device even with IP location', async () => {
      ...
      const device3Match = matches.find(
        (m) => m.deviceId === testDevice3Id.toString(),
      );
    ...
    it('should match saved zone when alert is within zone radius', async () => {
      ...
      const matches = await locationService.findDevicesForAlert(
        richmondAlert.id,
      );

      const savedZoneMatch = matches.find(
        (m) =>
          m.deviceId === testDevice1Id.toString() && m.matchReason === 'MANUAL',
      );
